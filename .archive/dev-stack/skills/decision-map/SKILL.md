...
Decision tickets live where `docs/agents/issue-tracker.md` says — the tracker, or `.scratch/<feature>/`. If the configuration is missing, ask the user once — the same ask-once rule as the tier-2 preflight — and record the answer in `docs/agents/issue-tracker.md` so nothing downstream asks again.
