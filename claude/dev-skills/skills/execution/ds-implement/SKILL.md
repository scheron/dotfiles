...
**Announce at start:** "Using ds-implement to execute the plan."
...
The format is in [environment-contract.md](environment-contract.md).
...
`ds-finish` reads the base off it; `finish-guard` arms itself on its existence.
...
4. the path to [tdd.md](tdd.md), if the plan says this segment has tests;
...
([CRITERIA.md](../../shared/ds-review-criteria/CRITERIA.md) — resolve it and pass
the absolute path).
...
|---|---|---|---|
| implementer | assigned by the plan | the segment's phases, TDD where the plan says there are tests; static checks — lint, typecheck, build | E2E, runtime, curl to an endpoint; is not required to report test numbers |
| `implement-review` | Sonnet, cold context | **runs the checks itself first**, then reads the diff against the phases' fields; patterns, conventions, smells, superfluous entities; judges the tests | does not debug stack traces or build noise — hands red straight back; does not compare the diff to the step list |
| `final-review` | Opus, cold context | does the system work: runtime, E2E, the plan's scenarios, snapshots, builds, clicking through | does not review code quality — that was closed at the checkpoints |
...
Per-agent worktree isolation is rejected: it does not remove the conflict, it
moves it into a merge that surfaces after both pieces of work are finished, with
...
Dispatch **`final-review`** on Opus with a cold context, giving it: the plan's
...
not review the fix locally; that is `implement-review`'s work.
...
`final-review` builds that list, since it makes one for itself anyway.

Invalidating every approval after any code change is rejected: by the third round
the human starts waving it through, and a strict rule degenerates into a
formality.

## Handoff

GATE 1 green → stop. The human invokes `ds-finish`.
