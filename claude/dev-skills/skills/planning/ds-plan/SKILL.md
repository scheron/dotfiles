...
**Announce at start:** "Using ds-plan to write the implementation plan."

**A plan is always created, and always as a file.** There is no inline mode. It
is a ledger as well as a contract — its checkboxes have to live somewhere, and a
run of several segments will almost certainly outlive a compaction that takes an
inline plan with it. Length is set by the task; a short task gives a short file.
...
execution, by `ds-implement`, not here.

In sequential work with nothing deferred, the base almost never moves between
planning and execution. Creating a workspace up front means paying a branch for
every idea that is not approved yet, against a risk near zero.
...
That, and `ds-finish` marking it *done*, are the only writes to a spec after it
exists.
...
| Section | Why | Read by |
|---|---|---|
| **Goal** | what we are doing, in your own words | everyone; `ds-finish` derives the commit message from it |
...
commit reads the goal and the result and writes "added such-and-such", instead of
reconstructing the meaning from a diff. Writing commit text in advance is work
nobody needs.
...
The opposite extreme is rejected with it. A plan that carries function bodies and
test code as literals makes the planner pay the full price of writing code they
cannot run, degrades the implementer into a transcriber, and goes stale at the
first factual correction — after which it lies to the human at every gate.
...
reviewer may not change with an ordinary finding. No separate contract section is
written: it would be a second copy of the same text and would diverge from the
phases at the first correction.
...
`CLAUDE.md`; see `ds-implement`'s `environment-contract.md`.
...
is in [codebase-design.md](codebase-design.md).
...
Once approved, stop. The human invokes `ds-implement`; it creates the workspace,
