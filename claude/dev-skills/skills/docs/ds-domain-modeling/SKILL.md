---
name: ds-domain-modeling
description: Build and sharpen a project's domain model — the glossary, and an ADR when one is earned. Invoke it directly to pin down vocabulary or record a decision; ds-grill-with-docs also reaches for it as terms settle.
---
...
The human invokes it directly, and `ds-grill-with-docs` reaches for it as terms settle. Bare `ds-grill` does not — the whole difference between those two skills is that one leaves a durable artifact and the other does not, and an automatic call here erases it.
