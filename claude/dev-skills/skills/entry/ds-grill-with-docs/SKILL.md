...
description: Grilling that also captures what outlives the conversation — glossary terms as they settle, and an ADR when one is genuinely earned. Invoke instead of ds-grill when the work introduces domain vocabulary or a hard-to-reverse decision.
disable-model-invocation: true
---

# Grilling, with the domain captured

Same interview as `ds-grill`, plus the part that survives the session.

**Announce at start:** "Using ds-grill-with-docs — grilling, and capturing the
domain as we go."

## Run the interview

Invoke **`ds-grill`** and follow it end to end: the process, the depth rule, the
approaches, the design sections, the stop at the end. It is the single home of
that text and this skill does not restate it.
...
only in this transcript. Add it to `CONTEXT.md` through `ds-domain-modeling`,
...
`ds-grill` produces alignment and no artifact. This one produces a durable
artifact. That difference in what comes out the other end is the only reason two
skills exist rather than one, which is why `ds-domain-modeling` may fire inside
this skill and must not fire inside bare `ds-grill`. Let it fire there and the
