...
**Announce at start:** "Using ds-tests. Nothing in production behaviour changes."
...
goes through `ds-bug` on its own route. Pin the current behaviour and leave it.
...
`ds-plan`, with these carried explicitly:
...
- **the seams**: existing ones, at the highest level that still pins the
  ...
  that is a `ds-refactor`, not a phase here;
