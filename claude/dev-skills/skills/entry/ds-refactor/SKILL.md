...
**Announce at start:** "Using ds-refactor. The contract here is that behaviour
does not change."

## Why it is called refactor and not improve

"Improve" carries a judgement about what is better, and an entry named after a
judgement invites scope. Halfway through, a rename becomes a redesign, and the
one thing that made this route cheap — that you can prove nothing changed — is
gone.
...
1. **Map it** if the territory is unfamiliar — `ds-scout`. You cannot promise
   invariance over code you have not read.
2. **Grill only if there is a decision.** Which target pattern, which library,
   which of two structures — that is a decision and it goes through `ds-grill`.
   Where the target is obvious, skip it.
3. **Plan it** — `ds-plan`, with three things the ordinary plan does not carry:
...
4. **Build it** — `ds-implement`, like anything else.
