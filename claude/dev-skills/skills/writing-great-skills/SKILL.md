...
Use this reference as an audit. A skill-editing pass is complete when invocation,
description, steps, reference placement, completion criteria, leading words, and
pruning have each been checked deliberately.

## Audit order

1. Pick **model-invoked** or **user-invoked**.
2. Rewrite the **description** for the chosen invocation.
3. Put **steps** before reference, and give each step a checkable
   **completion criterion**.
4. Push branch-only **reference** behind clear **context pointers**.
5. Prune **duplication**, **sediment**, and **no-ops**.
6. Replace repeated explanatory phrases with a stronger **leading word** when it
...
- **No-op** — a line the model already obeys by default, so you pay load to say nothing. The test: does it change behaviour versus the default? A weak leading word (_be thorough_ when the agent is already thorough-ish) is a no-op; the fix is a stronger word (_relentless_), not a different technique. The test is model-relative and settled by transcript, never by argument — [`TESTING.md`](references/TESTING.md).
